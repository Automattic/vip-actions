...
var require_promise_limit = __commonJS({
  "node_modules/promise-limit/index.js"(exports, module2) {
...
async function getAllGitHubItems(octokitCallable, params) {
  const items = [];
...
async function isPullRequestApproved(pullRequest, organization, repository) {
  const reviews = await getAllGitHubItems(getOctokit2().rest.pulls.listReviews, {
    owner: organization,
    repo: repository,
    pull_number: pullRequest.number
  });
...
async function callMarkAutoMergePullRequestEndpoint(pullRequest) {
  ...
  await getOctokit2().graphql(query, variables);
  return true;
}
